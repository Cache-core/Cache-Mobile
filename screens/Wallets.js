        ...
        {layout.userLoaded && walletsList.length === 0
          ? (<View style={styles.emptyWalletsWrapper}>
            <Text style={styles.emptyWalletsText}>
              You have no wallets currently. Please create a wallet by clicking on the + button, to start using Cache Mobile.
            </Text>
          </View>)
          : (<FlatList
            data={walletsList}
              ...
              <View style={(item.addr === appData.common.selectedWallet) ? [styles.flatview, styles.walletSelected] : styles.flatview}>
                <TouchableOpacity onPress={() => switchWallet(item.address)}>
                  <View>
                    ...
                    <Text style={styles.balance}>Balance: {item.total ? item.total.toLocaleString(undefined, format4Decimals) : 0} CXCHE</Text>
                    <Text style={(item.locked && item.locked > 0) ? [styles.data, styles.lockedText] : styles.data}>Locked: {item.locked ? item.locked.toLocaleString(undefined, format4Decimals) : 0} CXCHE</Text>
