...
const CreateDepositScreen = () => {
  ...
  if (state.appData.createDeposit.amount) {
    sendSummaryList.push({
      value: `${parseLocaleNumber(state.appData.createDeposit.amount, true).toLocaleString(undefined, format5Decimals)} CXCHE`,
      title: 'Deposit Amount',
      icon: 'md-cash'
    });

    sendSummaryList.push({
      value: `${appSettings.defaultFee} CXCHE`,
  ...
  if (state.appData.createDeposit.duration && state.appData.createDeposit.amount) {
    sendSummaryList.push({
      value: `${getDepositInterest(parseLocaleNumber(state.appData.createDeposit.amount, true), state.appData.createDeposit.duration).toLocaleString(undefined, format5Decimals)} CXCHE`,
  ...
  const getAmountError = () => {
    ...
    if ((amountAsFloat < 1) && (state.appData.createDeposit.amount)) {
      return "Amount must be at least 1 CXCHE"
      ...
      <View style={styles.walletWrapper}>
        ...
        >
          <View style={styles.fromWrapper}>
            ...
            <Text style={styles.fromBalance}>{currWallet.balance.toLocaleString(undefined, format4Decimals)} CXCHE</Text>
            {currWallet.locked
              ? (<View style={styles.lockedWrapper}>
                ...
                <Text style={currWallet.locked ? [styles.worthBTC, styles.lockedText] : styles.worthBTC}>
                  {`${currWallet.locked.toLocaleString(undefined, format4Decimals)} CXCHE'`}
                </Text>
              </View>)
              : null}
          </View>
        ...
        <Tips
          ...
          text="Here you can enter the amount of CXCHE to deposit..."
          onRequestClose={() => setGuideState(guideNavigation.next())}
        >
          <View style={styles.amountWrapper}>
            <ConcealTextInput
              ...
              rightIcon={
                <Text style={styles.ccxUnit}>CXCHE</Text>
              }
            />
          </View>
        </Tips>
        <View style={styles.amountPercentWrapper}>
          <ConcealButton
            ...
            onPress={() => setAppData({ createDeposit: { amount: ((currWallet.balance - appSettings.defaultFee) * 0.25).toLocaleString(undefined, format5Decimals) } })}
            text="25%"
          />
          <ConcealButton
            style={styles.btnDepositPercent}
            onPress={() => setAppData({ createDeposit: { amount: ((currWallet.balance - appSettings.defaultFee) * 0.50).toLocaleString(undefined, format5Decimals) } })}
            text="50%"
          />
          <ConcealButton
            style={styles.btnDepositPercent}
            onPress={() => setAppData({ createDeposit: { amount: ((currWallet.balance - appSettings.defaultFee) * 0.75).toLocaleString(undefined, format5Decimals) } })}
            text="75%"
          />
          <ConcealButton
            style={styles.btnDepositPercent}
            onPress={() => setAppData({ createDeposit: { amount: (currWallet.balance - appSettings.defaultFee - appSettings.minValue).toLocaleString(undefined, format5Decimals) } })}
            text="100%"
          />
        </View>
        <Tips
