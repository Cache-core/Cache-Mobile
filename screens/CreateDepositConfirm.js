...
const CreateDepositConfirm = () => {
  ...
  addSummaryItem(`${parseLocaleNumber(state.appData.createDeposit.amount, true).toLocaleString(undefined, format5Decimals)} CXCHE`, 'You are depositing', 'md-cash');
  addSummaryItem(`${getDepositInterest(parseLocaleNumber(appData.createDeposit.amount, true), appData.createDeposit.duration).toLocaleString(undefined, format5Decimals)} CXCHE`, 'Interest you will earn', 'md-cash');
  addSummaryItem(`${appData.createDeposit.duration} month${state.appData.createDeposit.duration > 1 ? 's' : ''}`, 'For a duration of', 'md-clock');
  addSummaryItem(`${appSettings.defaultFee} CXCHE`, 'Transaction Fee', 'md-cash');
