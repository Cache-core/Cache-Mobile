...
class LocalStorage {
  ...
  _initialize = (storage) => {
    SecureStore.getItemAsync('Cache.allKeys').then((keys) => {
      if (keys) {
  ...
  _storeAllKeys = (storage) => {
    ...
    SecureStore.setItemAsync('Cache.allKeys', allKeys.join()).then((result) => {
      // do nothing
    }).catch((err) => {
      console.log(err);
    });
  ...
  get = (name, defValue) => {
    let value = this.memStorage[`Cache.${name}`];
